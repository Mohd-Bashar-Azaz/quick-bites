# QuickBite — Technical Specification
...
|-----------|--------|-------|
| AppShell | Custom | 430px max-width container, page-level scroll management, shared background |
...
- `quickbite_cart`: serialized cart items
- `quickbite_user`: user profile + preferences
- `quickbite_onboarded`: boolean flag (skips onboarding on returning visit)
...
**Mobile-first container**: The entire app renders within a 430px max-width centered container simulating a mobile viewport. This is a demo/pitch prototype, not a responsive PWA.

**Authentication stub**: Login/OTP flow is not implemented. The app starts at Splash → (optionally Onboarding) → Home. The user is always "Haris Khan" from mock data.
