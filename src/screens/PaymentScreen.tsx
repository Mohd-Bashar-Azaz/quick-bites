...
const paymentMethods = [
  { id: 'gpay', name: 'Google Pay', detail: 'haris@oksbi', icon: '💳', type: 'upi' },
  { id: 'phonepe', name: 'PhonePe', detail: 'haris@ybl', icon: '💳', type: 'upi' },
  { id: 'paytm', name: 'PayTM Wallet', detail: 'Balance: ₹45', icon: '💰', type: 'wallet' },
  { id: 'qbwallet', name: 'QuickBite Wallet', detail: 'Balance: ₹120', icon: '⚡', type: 'wallet' },
  { id: 'counter', name: 'Pay at Counter', detail: 'Show token at pickup', icon: '🏪', type: 'counter' },
];

export default function PaymentScreen() {
  const { state, goBack, navigate, cartTotal, dispatch } = useApp();
  ...
  const handlePay = async () => {
    ...
    // Generate token
    const token = `A-${String(Math.floor(Math.random() * 99) + 1).padStart(3, '0')}`;

    setTimeout(() => {
      dispatch({ type: 'SET_ACTIVE_ORDER', orderId: `ord-${Date.now()}`, token });
  ...
  return (
    <div className="h-full flex flex-col bg-[#0F0F0F]">
      ...
      {/* Pay Button */}
      <div className="flex-shrink-0 p-4 bg-[#0F0F0F]">
