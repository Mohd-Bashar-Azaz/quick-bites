...
export default function CanteenDetailScreen() {
  ...
  const canteenItems = menuItems.filter(m => m.canteenId === canteen.id);

  const filteredItems = useMemo(() => {
    let items = canteenItems;
    ...
    return items;
  }, [canteenItems, activeCategory, searchQuery]);

  const groupedItems = useMemo(() => {
    const groups: Record<string, typeof canteenItems> = {};
  ...
  const handleAdd = (itemId: string, name: string) => {
    addToCart(itemId);
                        ...
                        <AnimatePresence mode="wait">
                          {qty > 0 ? (
                            ...
                            <motion.button
                              ...
                              exit={{ scale: 0.8, opacity: 0 }}
                              whileTap={{ scale: 0.85 }}
                              onClick={() => item.inStock && handleAdd(item.id, item.name)}
