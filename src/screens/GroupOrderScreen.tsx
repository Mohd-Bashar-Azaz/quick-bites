...
export default function GroupOrderScreen() {
  ...
  const [sharedItems] = useState([
    { id: 'gi1', name: 'Cheese Burger', qty: 2, price: 120, addedBy: 'Haris', avatar: 'H', color: 'from-orange-500 to-red-500' },
  ...
  if (!hasGroup) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#0F0F0F] px-8">
        <motion.div
  ...
  return (
    <div className="h-full flex flex-col bg-[#0F0F0F] overflow-y-auto no-scrollbar">
      ...
      <div className="pt-4 px-4 pb-3">
        ...
        <p className="text-xs text-[#A0A0A0] mt-0.5">Main Canteen • {groupParticipants.length} people ordering</p>
      </div>
      ...
      <div className="mx-4 bg-card rounded-2xl p-3 flex items-center gap-2">
        <div className="flex-1 bg-card-elevated rounded-xl px-3 py-2">
          <p className="text-[10px] text-[#6B6B6B] truncate">quickbite.app/g/haris-group-42</p>
        </div>
        <motion.button
          whileTap={{ scale: 0.92 }}
        ...
        <div className="flex items-center">
          {groupParticipants.map((p, i) => (
            ...
            >
              {p.avatar}
              {p.isHost && (
                <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-[#0F0F0F] flex items-center justify-center">
