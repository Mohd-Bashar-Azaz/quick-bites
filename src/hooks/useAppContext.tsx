...
import { menuItems, pastOrders, userProfile } from '@/data/mockData';
...
function appReducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'COMPLETE_ONBOARDING':
      return { ...state, isOnboarded: true, screen: 'home' };
    case 'SHOW_TOAST':
      return { ...state, toast: { message: action.message, type: action.toastType } };
    case 'HIDE_TOAST':
      return { ...state, toast: null };
    case 'REORDER': {
      const items = action.order.items.map(i => {
        const fullItem = menuItems.find(m => m.id === i.id);
        return fullItem ? { ...fullItem, quantity: i.quantity } : null;
